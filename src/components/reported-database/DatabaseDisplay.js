...
function DatabaseDisplay() {
    ...
    useEffect(() => {
        const fetchAllReportedUrls = async () => {
            try {
    ...
    const rows = reportedUrls.map((url, index) => ({
        ...
        reportedOn:
            url.ReportedOn instanceof Timestamp
                ? url.ReportedOn.toDate().toLocaleString()
                : '',
            ...
            <div className="bg-white flex flex-col items-center">
                <div>
                    ...
                    <p className="mt-3 text-sm opacity-70 text-center mx-[24%]">
                        Below is the view of all the URLs reported by our users
                        and also those which were classified as malicious by our
                        classification system. Help us maintain a secure
                        environment by contributing to this database.
                    </p>
                </div>
            </div>
