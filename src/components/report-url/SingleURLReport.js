...
const SingleURLreport = ({
    ...
    const reportSingleURL = async () => {
        ...
        if (singleURLInput.length < 5 || selectedCategory == '') {
            ...
            setSnackbarState(
                'Please enter a valid URL and select Category !',
                'warning',
                true
            );
            return;
        ...
        const isReported = await reportURLtoDatabase(
            singleURLInput,
            selectedCategory,
            reportedBy,
            urlSource,
        ...
        if (isReported) {
            ...
            setSnackbarState(
                'Failed to Report URL, try again !',
                'error',
                true
            );
        }
        ...
        <>
            ...
            <div className="mt-6 mx-[30%] flex flex-col border-2 rounded-md border-slate-300">
                <p className="ml-7 mt-3">
                    {' '}
                    Additional URL details (optional) :
                </p>
                <textarea
                    maxLength={10000}
